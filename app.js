    ...
    if (page === 'home') {
        ...
        app.innerHTML = `
            <div class="space-y-6 slide-in">
                <button id="pwa-btn" onclick="installApp()" class="${deferredPrompt ? '' : 'hidden'} w-full bg-slate-900 text-white p-4 rounded-2xl font-bold flex items-center justify-center gap-3">
        ...
        app.innerHTML = `
            <div class="space-y-6 slide-in">
                <div id="chart-container" class="bg-white p-4 rounded-[2rem] shadow-sm hidden">
    ...
    box.innerHTML = `
        <div class="p-6 bg-slate-50 rounded-3xl border border-slate-100 space-y-4 text-left">
            ...
            <div class="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200">
                <div><p class="text-[9px] font-black text-slate-400 uppercase">${t.res_tdee}</p><p class="text-lg font-black text-slate-800">${res.tdee} <span class="text-[9px] font-normal italic text-slate-400">kcal</span></p></div>
                <div><p class="text-[9px] font-black text-slate-400 uppercase">${t.water_title}</p><p class="text-lg font-black text-blue-600">${waterT} <span class="text-[9px] font-normal italic text-slate-400">ml</span></p></div>
            </div>
            <button onclick="shareWA()" class="w-full bg-[#25D366] text-white p-4 rounded-2xl font-bold flex items-center justify-center gap-2 mt-2 active:scale-95 transition shadow-lg">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    sel.addEventListener('change', (e) => { state.lang = e.target.value; save(); router(window.currentPage || 'home'); });
    router('home');
});
